...
- 원본 반영은 변경 범위와 공개 안전성을 확인한 뒤 patch/cherry-pick/import 방식으로 진행합니다. 무작정 대량 덮어쓰기는 하지 마세요.
- GitHub Desktop에 `Stashed changes`가 보이면 Restore하지 마세요. `.codex`, `tmp` 중심의 과거 로컬 검증 산출물일 가능성이 큽니다. 필요하면 내용을 확인한 뒤 Discard만 고려하세요.
