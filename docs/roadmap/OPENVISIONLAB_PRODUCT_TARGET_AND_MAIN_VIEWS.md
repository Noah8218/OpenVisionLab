...
1. P273 is complete for a reproducible unsigned portable Release Candidate gate. Two independent clean clones of commit `38e7eec8188b494b1c3f5d81a82cefa1ee9d19fe` produced the same 75-file, zero-PDB framework-dependent win-x64 package and exact ZIP SHA-256; Debug/Release, readiness, public-asset, all 33 public sample rows, archive verification, and copied-location launch passed. See `docs\reports\OPENVISIONLAB_PRODUCTION_RELEASE_GATE_20260730.md`. This is not commercial GA.
2. Separate immutable installation files from writable user/Recipe/log data, including explicit per-user/shared scope, migration, backup, reset, and stale-state behavior. This must precede a protected `Program Files` installer. Recommended model: `gpt-5.6-sol` | Reasoning effort: high.
3. Select the distribution/installation model and obtain the code-signing identity and certificate before installer, signed binary, update/rollback, and uninstall implementation. Prerequisite: approved distribution model, publisher identity, and signing certificate. Recommended model: none until prerequisites; `gpt-5.6-sol` afterward | Reasoning effort: none until prerequisites; high afterward.
