...
1. P275 is complete for the user-confirmed GitHub source-build goal. The root README gives one fresh-clone path; `tools\VerifySourceBuild.ps1` verifies exact SDK `8.0.421`, locked restore, Debug/Release, readiness, vendored DLLs, and expected EXEs. A real Windows 10 Sandbox replay passed the same checks after removing a Sandbox-only PowerShell Archive dependency, and the launcher left no Sandbox process. See `docs\reports\OPENVISIONLAB_GITHUB_SOURCE_BUILD_EXPERIENCE_20260731.md`.
