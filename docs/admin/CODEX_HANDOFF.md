...
- PropertyGrid는 사용자 요청에 따라 동결 대상이다. Threshold UX 안정화 이후 PropertyGrid row/editor/visibility/trackbar 동작은 이번 WPF 전환 작업에서 수정하지 않는다.
- WPF Tool View 범위: Filter, Morphology, EdgeDetection, Histogram, HSV, Mean, Rotate/Scale, Arithmetic이 WPF-hosted surface로 동작하고, 기본 WPF shell에서는 이 전처리 계열이 별도 native WPF tool window로 열린다. Shell 중앙은 메인 이미지 workspace, 하단은 기존 `OpenVisionLab.Logging.Controls`의 실제 `LogPanelView`로 유지한다. Filter/Morphology도 SimplePreprocess/Arithmetic과 같은 compact title header를 적용했고, localized title smoke를 추가했다.
- WPF 공통 UX: tool view combo/slider/checkbox/action button/output-layer icon, shell preview combo/card palette, shell navigation compact badge(`속성`/`흐름`), log panel dark combo template, AI Recipe toolbar status tooltip/accessibility를 보강했다.
- WPF shell preview의 레이어/결과 행은 선택 시 teal 배경/테두리로 더 분명하게 표시되며, smoke가 실제 WPF 버튼 command/parameter 경로로 `Main`/`NewPanel_sbA` 선택 후 중앙 캔버스와 상태바가 바뀌는지 확인한다.
- 검증 확장: `tools\RunUiPrecheck.ps1 -WpfTools`가 기본 UI precheck에 WPF shell preview, WPF Tool View screenshots, PropertyGrid-preserved floating tool route, Add Pipeline parity, tool-window factory, localization catalog checks를 추가한다. `tools\RunVisionPlatformPrecheck.ps1 -WpfTools`도 같은 옵션을 전달하고 summary JSON에 `WpfTools=true`를 기록한다.
- 최신 전체 검증: `tools\RunVisionPlatformPrecheck.ps1 -FailOnUiWarn -WpfTools -OutputDir artifacts\platform_precheck_wpf_full_cycle196` 통과. Build, Image Compare Coordinate Contract, Vision UI Contract, History Contract, Localization Catalog Contract, OpenVision Readiness Contract, XML Compatibility, Sample Catalog Runner, Runner API Contract, AI Recipe Interactive Contract, Tool Result Contract, Sample Inventory/Algorithm Contract, Tutorial Portable Contract, UI Precheck 모두 OK이며 UI summary JSON도 플랫폼 게이트에서 `Status=OK`, `WpfTools=true`, `OK/WARN/NG=27/0/0`으로 확인했다. Sample Catalog는 58 OK / 0 NG다.
- 최신 셸 UX 검증: `PipelineViewerScreenshotSmoke --target wpf_shell_preview artifacts\smoke\shell_layer_button_cycle115` 및 `tools\RunUiPrecheck.ps1 -Targets wpf_shell_preview -FailOnWarn -OutputDir artifacts\ui_precheck_shell_layer_button_cycle115` 통과. UI summary는 `Status=OK`, `OK/WARN/NG=1/0/0`.
- WPF Host Bridge 기준: WPF host는 `OpenVisionShellHostView`/`OpenVisionShellHostWindow`에서 기존 tool-window factory와 DisplayManager/DockPanel bridge를 사용해 레이어를 seed하고 남은 WinForms 도구 폼을 fallback 중앙 문서로 호스팅한다. 초기 bridge 검증 `PipelineViewerScreenshotSmoke --target wpf_shell_host_bridge artifacts\smoke\wpf_shell_host_window_cycle122` 통과, 이후 기본 실행은 WPF Shell로 승격했다.
- 최신 native WPF tool-window 진척: WPF host에서 Filter/Morphology/EdgeDetection/RotateScale/Mean/Arithmetic/HSV/Histogram은 더 이상 WinForms Form을 중앙 문서로 넣지 않고 `OpenVisionNativeToolDocument`를 통해 별도 native WPF Tool View 창으로 연다. Shell 중앙은 메인 이미지 workspace, 하단은 실제 `LogPanelView`로 유지하며, 전처리/알고리즘 뷰는 기존 작업 방식처럼 플로팅 창에서 조작한다. 사용자 화면에는 `WPF Host`/`Bridge` 같은 구현 문구를 노출하지 않는다. Native adapter는 layer list/input-output preview/output layer creation/preview execution/pipeline append를 직접 연결하며, 오른쪽 layer/result rail도 실제 DisplayManager 레이어(`EdgeDetection_Preview`, `RotateScale_Preview`, `Mean_Preview`, `Arithmetic_Preview`, `HSV_Preview`, `Histogram_Preview`, `Morphology_Preview`)를 표시한다. Mean preview는 측정 사각형/텍스트 overlay까지 결과 이미지에 렌더링하고, HSV는 색상 마스크 처리를 위해 원본 채널을 유지한다. 레이어/결과 rail은 preview 레이어가 많아지면 최신 결과로 스크롤된다. 최신 검증: `PipelineViewerScreenshotSmoke --target wpf_shell_host_native_tool artifacts\smoke\wpf_shell_log_panel_cycle151` 통과.
- 최신 PropertyGrid 보존 도구 UX: Blob/Contour/Line/Matching/FeatureMatching 같은 fallback 도구는 PropertyGrid editor를 그대로 유지하되, 기본 WPF shell에서는 `WindowsFormsHost`를 통해 `OpenVisionFloatingToolWindow` 안에 표시한다. Hosted mode에서는 내부 `VisionTestForm` title bar를 숨겨 WPF 창 title bar가 하나만 보이도록 한다. 플로팅 창의 최소화/최대화/닫기 툴팁도 언어 카탈로그 기반으로 갱신된다. 검증: `PipelineViewerScreenshotSmoke --target wpf_shell_host_property_tool artifacts\smoke\wpf_shell_property_tool_cycle158` 및 `tools\RunUiPrecheck.ps1 -Targets wpf_shell_host_property_tool -FailOnWarn -OutputDir artifacts\ui_precheck_wpf_property_tool_cycle159` 통과.
- 최신 직접 실행 상태 UX: 오른쪽 `직접 실행 결과` 패널은 도구 선택만으로 `OK`를 표시하지 않는다. 선택만 된 도구는 주황 계열 `대기` 배지/테두리와 `도구를 실행하면 결과가 표시됩니다.`를 보여주고, native WPF preview 또는 WinForms fallback 실행 결과가 들어온 뒤에만 녹색 `OK/완료`로 바뀐다. 기본 WPF host와 shell preview 모두 같은 기준을 쓴다. 검증: `tools\RunUiPrecheck.ps1 -Targets wpf_shell_host_native_tool,wpf_shell_host_property_tool,wpf_shell_preview -FailOnWarn -OutputDir artifacts\ui_precheck_wpf_direct_status_color_cycle165` 통과.
- fallback 실행 검증 보강: `wpf_shell_host_property_tool` smoke가 Blob 도구의 실제 `실행` 버튼을 눌러 WinForms fallback `PublishResult`가 WPF shell의 직접 실행 상태를 `대기 -> OK/완료`로 바꾸는지 확인한다. 최종 캡처는 다시 Blob 선택/대기 상태로 돌아오며, 하단 실제 로그뷰에는 Blob 실행 완료 이력이 남는다. 검증: `tools\RunUiPrecheck.ps1 -Targets wpf_shell_host_property_tool -FailOnWarn -OutputDir artifacts\ui_precheck_wpf_property_fallback_run_cycle167` 통과.
- 예상 경로 문구 보강: 오른쪽 `직접 실행 결과` 패널의 route 표기를 `경로:`에서 `예상 경로:`로 바꿔, 실행 전 선택 상태가 실제 결과 생성처럼 보이지 않도록 했다. 기존 실행 폴더의 `CONFIG\localization_catalog.tsv`가 오래된 문구를 유지해도 `Shell.RouteEmpty`/`Shell.RouteFormat` 기본값 마이그레이션으로 자동 갱신된다. 검증: `PipelineViewerScreenshotSmoke --target wpf_shell_host_property_tool,wpf_shell_preview artifacts\smoke\wpf_shell_expected_route_cycle174` 및 `tools\RunUiPrecheck.ps1 -Targets wpf_shell_host_property_tool,wpf_shell_preview -FailOnWarn -OutputDir artifacts\ui_precheck_wpf_expected_route_cycle175` 통과.
- 실제 route 동기화 보강: WPF shell host의 `예상 경로`는 더 이상 선택 도구명으로 만든 고정 `{Tool}_Preview`를 쓰지 않고, 활성 native WPF 도구 또는 PropertyGrid-preserved WinForms fallback 도구의 현재 입력/출력 콤보 값을 읽어 표시한다. 예: Blob fallback은 `Main -> Main`, HSV native preview는 `Main -> HSV_Preview`처럼 실제 선택값과 맞는다. 검증: `PipelineViewerScreenshotSmoke --target wpf_shell_host_property_tool,wpf_shell_host_native_tool,wpf_shell_preview artifacts\smoke\wpf_shell_dynamic_route_cycle176` 및 `tools\RunUiPrecheck.ps1 -Targets wpf_shell_host_property_tool,wpf_shell_host_native_tool,wpf_shell_preview -FailOnWarn -OutputDir artifacts\ui_precheck_wpf_dynamic_route_cycle177` 통과.
- WPF native tool 상태 문구 보강: `Output layer ready`와 `Pipeline add unavailable` 상태 prefix를 localization presenter에 추가했다. 출력 레이어 생성 후 HSV tool window가 `출력 레이어 준비됨 / HSV_Preview`로 표시되는 것을 확인했다. 검증: `PipelineViewerScreenshotSmoke --target wpf_shell_host_native_tool artifacts\smoke\wpf_native_status_localized_cycle178` 및 `tools\RunUiPrecheck.ps1 -Targets wpf_shell_host_native_tool -FailOnWarn -OutputDir artifacts\ui_precheck_wpf_native_status_localized_cycle179` 통과.
- WPF native tool 상태 색상 보강: status presenter가 원문과 localized 표시문을 함께 보고 성공/오류/확인 색상을 판정하도록 했다. `출력 레이어 준비됨 / HSV_Preview`는 성공 색상으로 표시된다. 검증: `PipelineViewerScreenshotSmoke --target wpf_shell_host_native_tool artifacts\smoke\wpf_status_color_tokens_cycle184` 및 `tools\RunUiPrecheck.ps1 -Targets wpf_shell_host_native_tool -FailOnWarn -OutputDir artifacts\ui_precheck_wpf_status_color_tokens_cycle185` 통과.
- 구현 용어 노출 방지 보강: Shell Host의 숨겨진 active document text는 WPF view type/`ActiveForm` 문자열 대신 사용자용 제목만 담도록 바꿨고, `BridgeStatus` 잔재는 `DirectStatus` 명칭으로 정리했다. UI smoke의 internal-text 진단도 `ToolWpfView`, `OpenVisionPipelineReviewView`, `ActiveForm=`, `WPF Host`, `BridgeStatus` 같은 구현 용어를 잡도록 확장했다. 검증: `tools\RunUiPrecheck.ps1 -Targets wpf_shell_host_bridge -FailOnWarn -OutputDir artifacts\ui_precheck_wpf_shell_direct_status_cycle190` 통과.
- Preview shell 툴팁 로컬라이즈: 상단 설정/내보내기/최소화/최대화/닫기 아이콘 툴팁을 고정 영어 문자열에서 localization binding으로 전환했고 `Common.Export` 키를 추가했다. Smoke는 한국어/영어 전환 후 툴팁이 카탈로그 값을 따르는지 확인한다. 검증: `tools\RunUiPrecheck.ps1 -Targets wpf_shell_preview -FailOnWarn -OutputDir artifacts\ui_precheck_wpf_shell_preview_tooltips_cycle194` 통과.
- 최신 WPF UI 묶음 검증: `tools\RunUiPrecheck.ps1 -WpfTools -FailOnWarn -OutputDir artifacts\ui_precheck_wpf_broad_cycle195` 통과. WPF shell preview, WPF Tool View screenshots, PropertyGrid-preserved floating tool route, Add Pipeline parity, tool-window factory, localization catalog checks를 포함한 27개 target 모두 OK이며 summary JSON은 `Status=OK`, `WpfTools=true`, `OK/WARN/NG=27/0/0`이다.
- 최신 WPF shell right-rail 진척: `OpenVisionShellHostView`의 레이어/결과 목록은 선택 가능한 WPF ListBox로 바뀌었고, 행 클릭 시 해당 DisplayManager layer가 활성화된다. Pipeline rail 상단에는 선택 레이어 썸네일, 이미지 크기, 최근 tack time, 표시 상태가 나타난다. 최신 검증: `PipelineViewerScreenshotSmoke --target wpf_shell_host_bridge artifacts\smoke\wpf_shell_right_rail_detail_cycle131` 통과.
- 최신 WPF Pipeline Review 진척: `Pipeline` 메뉴는 기본 WPF shell에서 `OpenVisionPipelineReviewView`를 별도 WPF review window로 연다. 새 WPF 화면은 `Wpf/Views`, `Wpf/ViewModels`, `Wpf/Documents` 구조로 배치했고, 대응 ViewModel은 반드시 `OpenVisionPipelineReviewViewModel`처럼 `ViewModel` suffix를 사용한다. Review 문서는 active recipe pipeline을 읽어 `PipelineFlowView`, selected step/tool/state, input/output preview, branch reason, parameter summary, validation status/detail, explicit Run Review, selected-step result summary/detail, run-log context를 보여준다. Run Review는 `VisionPipelineExecutionService`를 사용하되 결과 이미지를 Review 문서 내부 캐시에만 보관해 main workspace layer를 직접 publish하지 않는다. Pipeline Review의 window title, section labels, run button/status, flow/validation/result/run-log labels는 localization catalog 기반으로 정리했고, shell selected-layer 빈 이미지 상태도 `이미지 없음` / `레이어 대기`로 localize했다. 최신 검증: `PipelineViewerScreenshotSmoke --target wpf_shell_host_bridge artifacts\smoke\wpf_pipeline_review_localized_cycle181` 및 `tools\RunUiPrecheck.ps1 -Targets wpf_shell_host_bridge -FailOnWarn -OutputDir artifacts\ui_precheck_wpf_pipeline_review_localized_cycle182` 통과.
- 최신 기본 실행 전환: `Program.Main()`의 기본 경로를 WPF `OpenVisionShellHostWindow`로 승격했다. 기존 WinForms `FormMainFrame`은 `OPENVISIONLAB_WINFORMS_SHELL=1`, `OPENVISIONLAB_LEGACY_WINFORMS_SHELL=1`, 또는 `OPENVISIONLAB_WPF_SHELL=0`일 때만 fallback으로 실행된다. 기본 창 title은 사용자에게 구현 방식을 노출하지 않는 `OpenVisionLab`이며, 최신 UX 검증은 `PipelineViewerScreenshotSmoke --target wpf_shell_host_native_tool artifacts\smoke\wpf_shell_log_panel_cycle151` 통과.
- 최신 단독 WPF 묶음 검증: `tools\RunUiPrecheck.ps1 -WpfTools -FailOnWarn -OutputDir artifacts\ui_precheck_wpf_broad_cycle195` 통과. UI summary는 `Status=OK`, `WpfTools=true`, `OK/WARN/NG=27/0/0`. Shell Host bridge 구현 용어 진단은 cycle190에서 `OK/WARN/NG=1/0/0`으로 확인했고, 최신 전체 게이트 기준 WPF UI 검증은 cycle196의 `UiPrecheck.OK/WARN/NG=27/0/0`이다.
- 최신 체크박스 테마 검증: `PipelineViewerScreenshotSmoke --target tool_filter_form artifacts\smoke\tool_filter_checkbox_cycle117` 및 `tools\RunUiPrecheck.ps1 -Targets tool_filter_form -FailOnWarn -OutputDir artifacts\ui_precheck_filter_checkbox_cycle117` 통과. UI summary는 `Status=OK`, `OK/WARN/NG=1/0/0`.
- 다음 우선순위: Blob/Contour/Line/Matching/FeatureMatching은 PropertyGrid editor를 유지한 채 WPF preview/review surface만 주변에서 개선한다.

## 0. 최신 명명 정리 기록

- 2026-06-19: PropertyGrid Threshold UX를 안정화했다. Threshold 하위 옵션은 display name prefix로 부모/자식 관계가 드러나고, 기존 `결과 반전` 중복 노출은 숨긴 뒤 trackbar inline `Invert`로 통합했다. `USE_THRESHOLD` / `USE_ADAPTIVE_THRESHOLD` 체크는 옵션 표시 상태만 바꾸며, 체크 즉시 preview를 강제 실행하지 않는다. 이 UX는 회귀/사용자 요청이 없으면 추가 재설계하지 않는 기준으로 고정한다.
- 2026-06-19: Pipeline input/output flow를 보강했다. placeholder 10x10 black layer는 runnable input 후보에서 제외하고, `Run Preview`는 main workspace를 변경하지 않으며, `Publish Result`만 선택 output layer를 main workspace에 반영한다. 검증 target: `pipeline_input_output_flow_check`, `pipeline_form_run_preview`.
- 2026-06-19: Priority 1~7 후속으로 공개 샘플 Good/Bad pair를 확장했다. `Fiducial_Solder` pair를 추가해 clear fiducial과 solder fiducial을 `ResultCount`, `AreaMax`, `BoundsWidthMax` 기준으로 분리하고, readiness gate를 12 pair groups / 27 pair rows로 상향했다.
- 2026-06-19: Acceptance metric failure 진단을 tool-specific tuning guidance로 보강했다. Contour/Blob/LineGauge/Matching/Feature/Mean/Threshold/Edge 계열 실패 문구가 metric value/target뿐 아니라 InputLayer, threshold polarity/value, morphology, ROI, score/line/mean 관련 조정 후보를 함께 제시한다.
- 2026-06-19: 1~7 최종 검증: Debug build OK, `OpenVisionReadinessCheck=OK`, full sample catalog `58 OK / 0 NG`, `RunVisionPlatformPrecheck.ps1 -SkipUi=OK`. Evidence: `artifacts\sample_catalog_20260619\sample_catalog_summary.json`, `artifacts\platform_precheck_20260619\platform_precheck_summary.json`.
- 2026-06-18: Pipeline Samples의 sample selection UX를 보강했다. 폼 크기를 `1120x740`으로 넓히고, catalog detail 영역에 `샘플 선택 가이드 / Sample Selection Guide` 패널을 추가해 추천 Pipeline, Step flow, 기대 Metric/Pair, 확인할 값, 실패 시 수정 포인트, 마지막 검사 상태를 선택 즉시 보여준다. 기존 Source/Expected preview와 Good/Bad grid는 유지했다. 검증: Debug build OK, `pipeline_samples_form=OK`, `pipeline_samples_film_pair_check_action=OK`, `OpenVisionReadinessCheck=OK`, `LocalizationCatalogCheck=OK`.
- 2026-06-18: AI Recipe에 `Pair 검사 / Check Pair`를 추가했다. 활성 catalog sample의 Good/Bad pair를 현재 편집 중인 XML로 직접 실행하고, `Good/Bad Pair Check`, overall OK/NG, sample별 metric/final layer/message, 다음 튜닝 액션을 AI Feedback/XML Patch 영역에 남긴다. Samples 폼과 AI Recipe가 같은 sample metric 판정을 쓰도록 `VisionPipelineSampleCheckService`를 공통화했다. 검증: Debug build OK, `ai_recipe_catalog_sample_check=OK`, `ai_recipe_suggested_fix_check=OK`, `OpenVisionReadinessCheck=OK`, `LocalizationCatalogCheck=OK`.
- 2026-06-18: AI Recipe `Apply + Preview` 이후 before/after 리뷰 요약을 추가했다. Safe Fix 적용 전 마지막 Preview 상태와 적용 후 Preview 상태를 비교해 `Before Safe Fix`, `After Safe Fix`, `Catalog Gate`, `Metric Compare`, `Visual Review`, 다음 액션을 `XML 수정 요청` 영역에 표시한다. Step grid refresh가 비교 문구를 덮지 않도록 비교 표시는 Preview 완료 후 UI 큐 마지막에 예약한다. 검증: `ai_recipe_suggested_fix_check=OK`, `ai_recipe_catalog_sample_check=OK`, `OpenVisionReadinessCheck=OK`, `LocalizationCatalogCheck=OK`.
- 2026-06-18: AI Recipe Safe Fix 상세 표시를 `Value before -> after` 형태로 보강했다. 단일 파라미터 수정은 현재값과 적용값을 표시하고, `MIN_AREA/MAX_AREA` 같은 다중 min/max swap도 `3000/100 -> 100/3000`처럼 표시한다. 기존 runtime catalog merge 정책을 고려해 새 key `AiRecipe.SafeFixDetail.BeforeAfterFormat`을 추가했다. 검증: `PipelineViewerScreenshotSmoke` build OK, `ai_recipe_suggested_fix_check=OK`, `LocalizationCatalogCheck=OK`, `OpenVisionReadinessCheck=OK`.
- 2026-06-18: AI Recipe 카탈로그 샘플 피드백에 Good/Bad 판단 문구를 추가했다. 선택된 sample catalog row의 `ExpectedReasonText`, `PairGroup`, `PairRole`을 AI Feedback에 노출해 사용자가 XML을 직접 읽지 않아도 Good 샘플은 기대 metric 안에 유지하고 Bad/ExpectedFailure pair는 설명 가능한 metric 차이로 분리해야 함을 알 수 있다. 검증: `ai_recipe_catalog_sample_check=OK`, evidence `C:\Users\Public\Documents\ESTsoft\CreatorTemp\openvisionlab_ai_recipe_goodbad_decision_20260618_rerun\ai_recipe_catalog_sample_check.png`.
- 2026-06-18: AI Recipe에 `Revert Fix`를 추가했다. Safe Fix와 Layer Flow 적용 시 직전 XML을 저장하고, 현재 XML이 적용 직후 상태와 같을 때만 되돌리기를 활성화한다. 수동 XML 편집이 발생하면 snapshot은 폐기된다. Revert는 이전 XML 복원 후 Validate를 수행하고 `Reverted Safe Fix` diff를 patch panel에 남긴다. 검증: `PipelineViewerScreenshotSmoke` build OK, `ai_recipe_suggested_fix_check=OK`, `OpenVisionReadinessCheck=OK`, `LocalizationCatalogCheck=OK`.
- 2026-06-18: Sample Catalog를 추가 확장했다. Pattern Matching은 `Contour_TemplateMatching_WrongTargetBoard`, `Contour_TemplateMatching_NoTargetSwitch`를 ExpectedFailure로 추가했고, FeatureMatching은 `Feature_ScoreDiscrimination` Good/Bad pair로 ScoreMax 기반 wrong-target/low-score 판단을 추가했다. 실 자재 이미지인 `Sample\MasterImage` 계열은 catalog와 sample tree에서 제거했다. Sample UX에는 `ExpectedReasonText`를 추가해 기대 metric의 근거를 표시하고, AI Recipe Safe Fix Diff에는 현재 catalog gate와 Good/Bad comparison을 남기도록 했다. Readiness 최소 기준은 11 group / 24 row로 상향했다. `openvisionlab_no_master_platform_precheck_20260618` 기준 Build/Readiness/Sample Catalog/AI Recipe/Tool Result/Tutorial Portable 모두 OK이며 Sample Catalog는 55 OK / 0 NG다.
- 2026-06-18: AI Recipe Apply Safe Fix 이후 `tbPatch`가 Applied Safe Fix Diff로 전환되도록 했다. 적용된 Safe Fix 목록, visual review 필요 여부, 변경된 XML 라인, 다음 확인 액션을 보여준다. `PipelineViewerScreenshotSmoke`의 `ai_recipe_suggested_fix_check`가 `Applied Safe Fix Diff`, `XML Diff`, `MIN_AREA/MAX_AREA` 표시를 검증한다.
- 2026-06-18: AI Recipe Safe Fix UX를 한 단계 더 보강했다. `safeFixDetailLabel`을 디자이너에 추가해 선택한 수정안의 대상 Step, parameter/change, 적용 값, 선택 상태, review policy를 즉시 보여준다. `Use` 체크 변경 시 caption count도 즉시 갱신된다. `ai_recipe_suggested_fix_check=OK`, `OpenVisionReadinessCheck=OK`, `LocalizationCatalog=OK`, Debug build 0 warning/0 error 기준으로 확인했다.
- 2026-06-18: AI Recipe Safe Fix UX를 추가 보강했다. Safe Fix caption은 전체 제안 수, 선택된 수, 시각 확인 필요 수를 보여주며, smoke contract가 이 caption을 검사한다. `OpenVisionReadinessCheck`는 sample catalog의 Good/Bad pair group을 실제 CSV 파싱으로 검증해 최소 6개 group/12개 row, metric bound, Good/Bad 양쪽 role, group별 단일 baseline pipeline 조건을 확인한다.
- 2026-06-18: 1~7 우선순위가 반복 과제로 되돌아오지 않도록 `tools/OpenVisionReadinessCheck`를 추가했다. 이 비UI 계약은 LLM Recipe safe-fix/Layer Flow, Tool sample diagnostics, Pipeline IN/OUT preview UX, WPG editor contracts, Localization catalog, Tutorial portable assets, Release/External reference policy, 완료/진행 문서 hygiene를 한 번에 확인한다. `RunVisionPlatformPrecheck.ps1`에도 `OpenVision Readiness Contract` 게이트로 연결했다.
- 2026-06-18: dev 트리에 다시 생긴 루트 `Resources` 이미지 잔여물을 재정리했다. 활성 소스 기준 `Properties.Resources.*` 참조가 없고, 이미지 참조가 `src/OpenVisionLab/Properties/Resources.resx`와 stale generated `src/OpenVisionLab/Properties/Resources.Designer.cs`에만 남아 있음을 확인한 뒤 루트 `Resources` 폴더와 두 리소스 파일을 삭제했다. `src/Libraries/OpenVisionLab.Localization/Resources`는 다국어 카탈로그 폴더라 유지했다.
...
- 2026-06-09: 로그 호출부가 너무 길어지는 문제를 줄이기 위해 `OpenVisionLogger`를 `CLog`로 축약했다. 파일은 `src/Libraries/OpenVisionLab.Logging/CLog.cs`, 공개 진입점은 `CLog.Write(...)` 기준이며, 메인/UI/공통/Lib.OpenCV/RJControls 호출부도 모두 `CLog.Write(...)`로 치환했다. `OpenVisionLab.sln` Debug / Any CPU 빌드에 성공했다.
- 2026-06-09: 기존 메인단/공통/비전툴 로그 호출을 `AppLog.*`에서 `OpenVisionLab.Logging` DLL 직접 호출로 전환했다. 호환 어댑터였던 `src/Libraries/Lib.Common/AppLog.cs`를 삭제했고, `Lib.Common`, `Lib.OpenCV`, `Lib.OpenCV.Blob`, 메인 프로젝트가 `OpenVisionLab.Logging` 프로젝트를 직접 참조한다. 메인 프로젝트의 `log4net.config` 직접 Content 복사 항목은 제거하고, `OpenVisionLab.Logging` 프로젝트의 content copy 전파에 맡겼다. 활성 소스 기준 `AppLog` 호출/클래스와 구 `Log4netView`/`CustomMemoryAppender` 잔재는 없다. `OpenVisionLab.sln` Debug / Any CPU 빌드에 성공했다.
...
- 2026-06-08: 루트 이미지 자산을 정리했다. 실제 실행 코드에서 `Properties.Resources` 이미지 리소스를 사용하지 않고, 남은 참조가 주석/프로젝트 리소스 등록뿐임을 확인한 뒤 `Image`, `Images`, `Resources` 폴더를 삭제했다. 함께 `src/OpenVisionLab/Properties/Resources.resx`, `src/OpenVisionLab/Properties/Resources.Designer.cs`와 `src/OpenVisionLab/OpenVisionLab.csproj`의 삭제된 `Images` 개별 Include 항목도 제거했다. `RJBaseForm`/`RJImageColorOverlay`에 남아 있던 이미지 리소스 주석과 빈 이벤트 핸들러 껍데기도 정리했다. 현재 소스 트리의 이미지 파일은 애플리케이션 아이콘 `lens.ico`만 남는다. `OpenVisionLab.sln` Debug / Any CPU 빌드에 성공했다.
...
- 2026-06-07: 사용하지 않는 옛 `src/Libraries/Controls.Viewer2D` 폴더를 삭제하고, `src/OpenVisionLab/OpenVisionLab.csproj`의 해당 폴더 제외 항목도 제거했다. 현재 실사용 뷰어 프로젝트는 `src/Libraries/OpenVisionLab.ImageCanvas`다. 삭제 후 `Controls.Viewer2D`/`ToolKit2DViewer`/`Editable2DViewer` 검색 결과는 없고, Debug / Any CPU 별도 출력 폴더 빌드에 성공했다.
...
- 2026-06-07: 프로젝트 전체에서 Cyotek `ImageBox` 의존성을 제거했다. `FormVision_Arithmetic`과 `FormMeasure`도 `VisionTestImageCanvas`로 전환했고, `LayerViewerState`는 ImageBox 호스팅 컴포넌트가 아니라 ROI 상태/DisplayManager 연결만 남긴 경량 컴포넌트로 축소했다. `DrawObject`의 `ImageBox` 타입 인자와 불필요한 `using Cyotek.Windows.Forms`를 제거했으며, `src/OpenVisionLab/OpenVisionLab.csproj`의 `Cyotek.Windows.Forms.ImageBox` 참조와 `dll/Cyotek.Windows.Forms.ImageBox.dll` 파일을 삭제했다. 소스/XAML/프로젝트 파일 기준 `rg "ImageBox"` 결과가 없고, Debug / Any CPU 별도 출력 폴더 빌드에 성공했다.
...
- 2026-06-07: `src/Libraries/OpenVisionLab.ImageCanvas`와 `FormImageEditView.cs` 기준으로 `Editable`, `NX`, `TX`, `Diagram` 계열 이전 명칭 검색 결과가 없도록 정리했다.
...
- 이미지 표시/ROI 편집/OpenGL 렌더링은 `src/Libraries/OpenVisionLab.ImageCanvas` 프로젝트에서 관리한다.
...
- `src/Libraries/OpenVisionLab.ImageCanvas/View/RoiImageCanvasView.xaml`
- `src/Libraries/OpenVisionLab.ImageCanvas/View/RoiImageCanvasView.xaml.cs`
- `src/Libraries/OpenVisionLab.ImageCanvas/ViewModel/RoiImageCanvasViewModel.cs`
- `src/Libraries/OpenVisionLab.ImageCanvas/RoiInteraction/RoiInteractionMouseDown.cs`
- `src/Libraries/OpenVisionLab.ImageCanvas/RoiInteraction/RoiInteractionMouseMove.cs`
- `src/Libraries/OpenVisionLab.ImageCanvas/RoiInteraction/RoiInteractionMouseUp.cs`
- `src/Libraries/OpenVisionLab.ImageCanvas/Compatibility/MvvmCompatibility.cs`
- `src/Libraries/OpenVisionLab.ImageCanvas/OpenGL/OpenGlDrawing.cs`
- `src/Libraries/OpenVisionLab.ImageCanvas/OpenGL/OpenGlRenderer.cs`
- `src/Libraries/OpenVisionLab.ImageCanvas/OpenGL/OpenGlTextureDrawingParam.cs`
- `src/Libraries/OpenVisionLab.ImageCanvas/Canvas/CanvasViewState.cs`
- `src/Libraries/OpenVisionLab.ImageCanvas/Engine/ImageCanvasControl.cs`
- `src/Libraries/OpenVisionLab.ImageSpace.Core/*`
- `src/Libraries/OpenVisionLab.Display.Core/*`
...
- 완료: 비활성/레거시 `src/Libraries/Controls.Viewer2D` 폴더는 산출물만 남은 상태로 확인 후 삭제했다.
...
- 완료: `src/Libraries/Controls.Viewer2D` 폴더는 현재 `.vs`, `bin`, `obj`, `.csproj.user` 산출물만 남아 있고 활성 소스/솔루션 참조가 보이지 않아 삭제했다.
...
- `src/Libraries/Controls.Viewer2D` 옛 폴더는 삭제됐다. 현재 실사용 프로젝트는 `src/Libraries/OpenVisionLab.ImageCanvas`다.
