...
| --- | --- | --- |
| `OPENVISIONLAB_CURRENT_HANDOFF.md` | First continuation brief. | Includes completed Recipe change-safety, explicit validation outcomes, Qualified Recipe Snapshot, completed `CVR-01` through `CVR-08`, and bounded CVR-09 LineFixture v1 implementation/synthetic evidence; `CVR-00` independent novice evidence remains the active external prerequisite. |
| `OPENVISIONLAB_COMMERCIAL_VIDEO_DEVELOPMENT_BACKLOG_20260727.md` | Full priority-ordered development inventory derived from all 16 reviewed Cognex/HALCON/MERLIC videos. | Canonical `CVR-00` through `CVR-20` queue with activation triggers, detailed minimum contracts, model/reasoning recommendations, source-video traceability, excluded scope, and durable evidence. CVR-09 reusable v1 is implemented while physical-task qualification remains pending; it preserves later candidates without auto-activation. |
