# OpenVisionLab Blob Tool View Partial retention — PL-0046

Status: `Complete`

## Scope

이번 cycle은 `BlobToolWpfView.xaml.cs`를 Blob 전용 interaction, property
state, teaching preview, area verification, shared single-input runtime,
factory, document, binding/test contract, registry, and lifetime owner와
...
| --- | --- | --- |
| XAML namescope, tool shell, Blob Learn caption/topic, verification-guide content | `BlobToolWpfView.xaml` + View Partial | Required WPF composition으로 유지. |
| Property-grid binding, layer/preview/output events, auto-preview, result review, presets, language, release | `VisionToolSingleInputPropertyToolController<BlobProperty>` | Existing generic controller가 shared facade와 disposal을 소유. |
| Mutable Blob parameters, range normalization, summary, property snapshot | `BlobToolViewModel` via `VisionToolCompositionService` | Blob property writer와 ViewModel creation을 소유. |
| Property-grid presenter, binding flush, settings callback | `VisionToolPropertyGridPresenter<BlobProperty>` + generic runtime | View는 presenter/controller를 조합하고 state를 복제하지 않음. |
| Debounce, property-change policy, persistence callback, preview state | `VisionToolSingleInputPropertyToolRuntime<BlobProperty>` | Shared runtime이 edit ordering과 lifetime을 소유. |
...
| Area teaching text, result metrics, verification status | `VisionToolAreaVerificationGuidePresenter<BlobProperty, BlobResult>` + `VisionToolAreaVerificationCriteriaText` | Inspection presentation policy가 View 밖의 existing presenter/criteria에 있음. |
| Blob settings/session and property-grid composition | `OpenVisionNativePropertyGridToolFactory` + `VisionToolCompositionService` | View Partial에는 session store 또는 persistence coupling이 없음. |
...
| Blob algorithm, result capture, overlay image | `OpenVisionNativeToolPreviewExecutor` + `OpenVisionNativeToolPreviewOverlayRenderer` | `BlobTool` construction과 algorithm policy가 View 밖에 있음. |
...
공개하는 narrow facade이며 Blob domain state나 persistence owner가 아니다.
...
```text
OpenVisionNativeToolRegistry (VISION_MENU.Blob)
  -> OpenVisionNativePropertyGridToolFactory.CreateBlob
  -> property-session store and VisionToolCompositionService.CreateBlobToolViewModel
  -> OpenVisionNativePropertyGridToolDocumentBuilder
  -> BlobToolWpfView + generic presenter/controller/runtime
  ...
  -> OpenVisionNativeToolPreviewExecutor.ExecuteBlobPreview
     -> BlobTool algorithm -> Blob result -> overlay renderer
...
`System.IO`, OpenCV/Blob algorithm construction, persistence store, or pipeline
policy was found.

## Binding, public, and test contract

- `BlobToolWpfView.xaml` retains its `x:Class`, `toolShell` namescope,
  `VectorCircle` icon, `Learn Blob` caption, topic index `5`, and compact
  `VisionToolVerificationGuideView` content.
- `ISingleInputPropertyVisionToolWpfView<BlobProperty>` remains the generic
...
collaborators, narrow facade methods, and Blob-specific metric selector wiring.
Mutable Blob policy, normalization, presentation, teaching preview state,
property binding, preview scheduling, settings persistence, document creation,
algorithm execution, pipeline routing, and shared runtime lifetime already have
concrete owners. A new Blob ViewModel/service/interface/manager or another
Partial would duplicate or hide those owners. The smallest correct structural
result is to retain the XAML Partial and protect its owner map with a source
contract.

## Shortest code-reading order

Search once for `BlobToolWpfView`, then read:

1. `src/OpenVisionLab/UI/VisionTest/Wpf/ToolViews/BlobToolWpfView.xaml.cs`
   and `.xaml` for namescope, controller attachment, facade, teaching, and
   result-review wiring.
2. `src/OpenVisionLab/UI/VisionTest/ViewModels/BlobToolViewModel.cs` and
   `src/OpenVisionLab/UI/VisionTest/Composition/VisionToolCompositionService.cs`
   for mutable state, range normalization, summary, and ViewModel creation.
...
- `BlobToolPartialBoundaryContract` passed 11/11 in Debug and Release.
- `VisionRecipeRunnerSmoke` Debug and Release builds passed with 0
  warnings/errors.
- `RunUiPrecheck.ps1` passed `wpf_shell_host_blob_tool` and
  `wpf_openvision_learn_blob` with `check=OK`, layout/text/internal counts all
  zero, and fresh screenshots.
