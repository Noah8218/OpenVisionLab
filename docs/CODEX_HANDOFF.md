...
- 2026-06-18: dev 트리에 다시 생긴 루트 `Resources` 이미지 잔여물을 재정리했다. 활성 소스 기준 `Properties.Resources.*` 참조가 없고, 이미지 참조가 `Properties/Resources.resx`와 stale generated `Properties/Resources.Designer.cs`에만 남아 있음을 확인한 뒤 루트 `Resources` 폴더와 두 리소스 파일을 삭제했다. `Library/OpenVisionLab.Localization/Resources`는 다국어 카탈로그 폴더라 유지했다.
