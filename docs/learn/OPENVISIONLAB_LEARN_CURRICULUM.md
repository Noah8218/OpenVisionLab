...
| 10 | Color image processing | HSV, color range segmentation, channel thinking, color drift | Partial: Learn window topic 16 and `LEARN_COLOR_HSV.md`; tools: HSV, Mean, Histogram; gap: HSV is preview-only until `HSV` VisionPipeline ToolType plus metric gate such as `MaskPixelRatio` exists; stable HSV Good/Bad sample and channel split/merge guide/tool remain open |
