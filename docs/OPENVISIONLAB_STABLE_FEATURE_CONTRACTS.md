...
- `wpf_shell_host_workspace_sample_pipeline_review_film_ng_metrics`
  - Legacy-named target retained for compatibility; covers opening public-safe `Public_Threshold_BandPads_Missing_Bad`, verifying controlled `ResultCount` NG baseline behavior, binding Pipeline Review to the active sample pipeline, explicit Run Review, beginner NG next action, result-count metric detail, run log, output preview, and no native Preview side effects before explicit review.
