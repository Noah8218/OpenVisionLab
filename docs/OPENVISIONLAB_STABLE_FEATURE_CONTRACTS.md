...
- `USE_MASKING` is a UI/editor visibility switch; toggling it must not run preview by itself.
...
- Good/Bad sample catalog coverage must keep representative pair groups for Blob, Contour/fiducial, LineGauge, Matching, EdgeBasedMatching, FeatureMatching, Mean, film defect, and surface defect flows. Each pair group must include both Good and Bad references, one shared baseline pipeline, bounded expected metrics, and at least one shared Good/Bad metric.
- A Bad reference may be `ExpectedFailure` when the shared baseline pipeline intentionally rejects the sample through a stable metric acceptance gate. `Mean_Brightness_DimBad` is a controlled NG reference: the Mean tool still produces `MeanValueAvg`, but the sample pipeline rejects values below the normal-brightness acceptance threshold.
- Feature score-discrimination Bad references are controlled NG references. `Feature_Template_Review.pipeline.xml` must gate acceptance on `ScoreMax` for the normal target range, so low-score/wrong-target Feature hypotheses can still produce a result image while Pipeline Review reports metric NG.
- LineGauge angle Bad references may be controlled NG references when the shared pipeline measures fitted rail angle. `Pins_Edge_LineGauge.pipeline.xml` gates acceptance on `LineAngleAvg` in the straight-rail range, so tilted rails can still produce edge/line overlays while Pipeline Review reports metric NG.
- Blob density Bad references may be controlled NG references when the shared pipeline measures particle count. `Rice_Particle_BlobPair.pipeline.xml` gates acceptance on `ResultCount` in the normal dense-particle range, so sparse-density samples can still produce Blob result images while Pipeline Review reports metric NG.
- BentPin shaft-width Bad references may be controlled NG references when the shared pipeline measures maximum shaft contour width. `BentPin_ShaftContour.pipeline.xml` gates acceptance on `BoundsWidthMax` in the normal shaft-width range, so bent shafts can still produce contour result images while Pipeline Review reports metric NG.
- Film dark-spot Bad references may be controlled NG references when the shared pipeline measures maximum dark-spot area. `Film_DarkSpot_Contour.pipeline.xml` gates acceptance on `AreaMax` in the normal small-background-candidate range, so larger dark spots can still produce contour result images while Pipeline Review reports metric NG.
...
- `wpf_shell_host_workspace_sample_pipeline_review_metrics`
  - Covers opening `Blob_RiceParticle_Good`, verifying catalog baseline metrics, binding Pipeline Review to the active `Sample_` pipeline, explicit Run Review, OK decision text, primary result metric, run log, output preview, and no native Preview side effects before explicit review.
- `wpf_shell_host_workspace_sample_pipeline_review_ng_metrics`
  - Covers opening `Mean_Brightness_DimBad`, verifying controlled `MeanValueAvg` NG baseline behavior, binding Pipeline Review to the active `Sample_` pipeline, explicit Run Review, beginner NG next action, metric detail, run log, output preview, and no native Preview side effects before explicit review.
- `wpf_shell_host_workspace_sample_pipeline_review_feature_ng_metrics`
  - Covers opening `Feature_TemplateReview_LowScoreSwitch`, verifying controlled `ScoreMax` NG baseline behavior, binding Pipeline Review to the active `Sample_` pipeline, explicit Run Review, beginner NG next action, score metric detail, run log, output preview, and no native Preview side effects before explicit review.
- `wpf_shell_host_workspace_sample_pipeline_review_line_ng_metrics`
  - Covers opening `LineGauge_PinsTilted_Bad`, verifying controlled `LineAngleAvg` NG baseline behavior, binding Pipeline Review to the active `Sample_` pipeline, explicit Run Review, beginner NG next action, angle metric detail, run log, output preview, and no native Preview side effects before explicit review.
- `wpf_shell_host_workspace_sample_pipeline_review_blob_ng_metrics`
  - Covers opening `Blob_Bacteria_SparseBad`, verifying controlled `ResultCount` NG baseline behavior, binding Pipeline Review to the active `Sample_` pipeline, explicit Run Review, beginner NG next action, Blob count metric detail, run log, output preview, and no native Preview side effects before explicit review.
- `wpf_shell_host_workspace_sample_pipeline_review_bentpin_ng_metrics`
  - Covers opening `BentPin_BadShaft`, verifying controlled `BoundsWidthMax` NG baseline behavior, binding Pipeline Review to the active `Sample_` pipeline, explicit Run Review, beginner NG next action, shaft-width metric detail, run log, output preview, and no native Preview side effects before explicit review.
- `wpf_shell_host_workspace_sample_pipeline_review_film_ng_metrics`
  - Covers opening the dark-spot Bad sample, verifying controlled `AreaMax` NG baseline behavior, binding Pipeline Review to the active sample pipeline, explicit Run Review, beginner NG next action, dark-spot area metric detail, run log, output preview, and no native Preview side effects before explicit review.
...
- `wpf_shell_host_workspace_sample_pipeline_review_line_ng_metrics`
  - Covers a real LineGauge Bad sample opening into Pipeline Review, active `Sample_` pipeline binding, explicit Review execution, controlled `LineAngleAvg` NG, beginner next action, angle metric detail, run log, output preview, and no native Preview side effects before explicit review.
- `wpf_shell_host_workspace_sample_pipeline_review_blob_ng_metrics`
...
- `wpf_shell_host_workspace_sample_pipeline_review_bentpin_ng_metrics`
  - Covers a real BentPin Bad sample opening into Pipeline Review, active `Sample_` pipeline binding, explicit Review execution, controlled `BoundsWidthMax` NG, beginner next action, shaft-width metric detail, run log, output preview, and no native Preview side effects before explicit review.
- `wpf_shell_host_workspace_sample_pipeline_review_film_ng_metrics`
  - Covers a real Film Bad sample opening into Pipeline Review, active `Sample_` pipeline binding, explicit Review execution, controlled `AreaMax` NG, beginner next action, dark-spot area metric detail, run log, output preview, and no native Preview side effects before explicit review.
...
- 2026-06-29 docking DLL boundary guard: AvalonDock package ownership starts in `Library/OpenVisionLab.Docking.Controls/OpenVisionLab.Docking.Controls.csproj`. Do not add `Dirkster.AvalonDock` back as a direct `OpenVisionLab.csproj` package reference. New docking policy/control code should move toward `OpenVisionLab.Docking.Controls`; ShellHost should consume it through wrapper/control APIs instead of growing more AvalonDock-specific code-behind. Validation: `dotnet list OpenVisionLab.csproj package --include-transitive` must show `Dirkster.AvalonDock` only as transitive, plus `dotnet build OpenVisionLab.sln -c Debug -p:Platform="Any CPU"` and `OpenVisionLab.exe --smoke layer-global-docking`.
...
- Probe validation on 2026-06-27 showed the template/model cache kept the same local rotation and synthetic probe results. The repeated rotation run still measured about 45-58 ms, so the next speed bottleneck is source gradient generation and position scanning, not template model creation.
...
- 2026-06-27 speed pass: source/template gradient unit vectors are precomputed and scoring uses array-based dot products. `Cv2.CartToPolar` was replaced with `Cv2.Magnitude` because angle output was unused. Focused probes kept the same score/angle/center outputs; the local coarse angle/refine probe measured about 32.6 ms in the sequential run. UI validation: `artifacts\ui_precheck_edge_based_unit_gradient_20260627`.
- 2026-06-27 sample rotation benchmark: `.codex\EdgeBasedSampleRotationBenchmark` validates 10 local rotation samples with inserted `0, +5, +10, -5, -10` degree targets. Pass criteria are center error <= 6 px and angle error <= 2 deg. Latest artifact: `artifacts\edge_based_hybrid_perf_20260627`.
...
- 2026-06-28 current recommendation recheck: `artifacts\edge_based_current_benchmark_20260628` compared default and large local rotation sets with baseline and pyramid variants. `EdgeHybrid` remains the safe edge-based recommendation. `EdgeOptimized` alone must not become the production default because repeated-pattern samples can still select wrong repeated structures without Hybrid verification. `Pyramid proposal top N = 6` remains an explicit operator option, not a silent default migration.
...
- Scale verification images must be generated by resizing the whole original sample image. Do not create composite/blurred/pasted test images for scale validation. Current focused probes use whole-image resized samples and validate target scale at 0.90x and 1.10x.
