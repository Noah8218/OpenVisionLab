...
- Native Tool correctness must not depend on background prewarm completing. When a PropertyGrid-heavy Tool document is created on demand, it must receive the same hosted-layout preparation before its first floating or docked presentation. Cancelling or disabling prewarm must not crash the Tool, run Preview/Run, create layers, change the active layer, or change input/output routing.
...
- Double-clicking a non-empty input/output preview keeps the inline fit reset and opens the same image in one reusable, resizable large viewer for the active Tool. The viewer title identifies the localized Tool, Input/Input A/Input B/Output role, and routed layer.
- Repeated double-clicks in the same Tool reuse that viewer while switching to the exact requested role. An explicit Preview refreshes an already open Output viewer; switching or closing the Tool closes the Tool-owned viewer.
- Opening, switching, refreshing, or closing the large viewer must not execute Preview/Run, create/select a layer, change the active layer, or alter input/output routing.
...
- Open an empty preview, create one large window per role, or leave a Tool preview viewer alive after its owning Tool changes.

Relevant smoke:
- Actual EXE `--smoke tool-preview-popout`
- `wpf_shell_host_blob_tool`
- `wpf_shell_host_contour_tool`
- `wpf_shell_host_line_measure_tool`
- `wpf_shell_host_matching_tool`
