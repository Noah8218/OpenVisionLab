...
2. Attach these two files together:
...
Do not overwrite `COPY_THIS_TO_GPT.txt` with GPT's response. Do not repair prose, Markdown fences, parameter names, or invalid XML before returning it. The unchanged first response is the evidence.
