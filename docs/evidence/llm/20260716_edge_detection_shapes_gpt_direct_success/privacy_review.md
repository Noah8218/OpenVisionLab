...
- The nominal and Missing-NG inputs are project-authored public synthetic assets registered in the public sample manifest.
...
- Both result images are 572x420 and contain only `IHDR`, `IDAT`, and `IEND` PNG chunk types. No text or EXIF metadata is present.
- Result images contain only the public synthetic shape scenes and OpenVisionLab EdgeDetection/Morphology/Contour output.
...
- Latest-EXE validation/import passed with 3 Steps, 0 errors, 0 warnings, no dependencies, and no import-time image run.
- The public nominal sample passed at `ResultCount=4` in 33.914 ms.
- The public Missing-NG sample produced the expected inspection NG at `ResultCount=2 < 4` in 30.826 ms.
