...
Status: Translation-only runtime, Recipe Manager PropertyGrid round trip, and public Good/Bad operator workflow implemented on 2026-07-14.
...
- `VisionToolResult` exposes metrics and overlays. Matching rectangle overlays carry a center and angle.
...
3. Review the match score, center, and angle.
4. Mark the Matching step as fixture frame `PartFrame` and store the reviewed reference X, Y, and angle.
...
8. Review `FixtureOffsetX`, `FixtureOffsetY`, `FixtureAngleDelta`, and the effective ROI metrics together with the inspection result.
...
- the frame name or numeric reference pose is missing;
...
Pipeline Review exposes `Save as reference` only after an explicit successful Review produces one valid Matching fixture pose. The command copies `FixtureCenterX`, `FixtureCenterY`, and `FixtureAngle` into the three reference fields, saves the active pipeline, invalidates the old review result, and requires another explicit Review. It does not launch Preview/Run itself.
