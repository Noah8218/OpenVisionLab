    ...
    public partial class FormViewer_MotionControl : MetroForm
    {
        ...
        public FormViewer_MotionControl(CPropertyMotion position)
        {
            ...
            if(pos != null)
            {
                ...
                try
                {
                    Axis = Global.Device.MOTION_AJIN.Axises[pos.AXIS_NO];
                    Status = Axis.Status;
                    Home = Axis.Home;
                }
                catch (Exception ex)
        ...
        public FormViewer_MotionControl()
        {
            InitializeComponent();
        ...
        private void btnSaveZigElevatorBufferCount_Click(object sender, EventArgs e)
        {
            try
            {   
                //Global.Data.PropertyBufferPitch.SaveConfig(Global.Recipe.Name);
                //Global.DATA.PropertyBufferPitch.LoadConfig(Global.Recipe.Name);
                CLogger.Add(LOG.NORMAL, "[OK] {0}==>{1}", MethodBase.GetCurrentMethod().ReflectedType.Name, MethodBase.GetCurrentMethod().Name);
            }
            catch (Exception ex)
            {
        ...
        private void btnStartHome_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    if (CUtil.ShowMessageBox("CHECK", "DO YOU WANT TO SET THE HOME?", FormMessageBox.MESSAGEBOX_TYPE.Quit))
                    {
