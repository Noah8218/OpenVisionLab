...
namespace OpenVisionLab
{
    ...
    internal static class OpenVisionPipelineReviewGuidePresenter
    {
        ...
        public static OpenVisionPipelineReviewGuideState CreateSelected(
            ...
            bool isBranch = false,
            bool inputWillBeProduced = false,
            OpenVisionWorkspaceSamplePairDecisionGuide samplePairGuide = null)
        {
            return new OpenVisionPipelineReviewGuideState(
                FormatStage(displayIndex, stepCount),
                FormatCurrentStep(displayIndex, step),
                ResolveNextActionText(displayIndex, stepCount, step, hasInputImage, hasOutputImage, summary, validationResult, inputWillBeProduced),
                ResolveResultDecisionText(statusText, summary, validationResult),
                ResolveDetailText(displayIndex, stepCount, step, hasInputImage, hasOutputImage, summary, validationResult, expectedInputLayer, isBranch, inputWillBeProduced),
        ...
        private static string FormatCurrentStep(int displayIndex, VisionPipelineStep step)
        {
            ...
            string ordinal = Math.Max(0, displayIndex).ToString("00", CultureInfo.CurrentCulture);
            ...
            bool inputWillBeProduced)
        {
            ...
            if (!hasInputImage && !inputWillBeProduced)
            {
            ...
            if (summary == null)
            {
                if (!hasInputImage && inputWillBeProduced)
                {
                    return T("PipelineReview.Guide.InputPendingNext", "Run Review to create this input from the earlier step");
                }
            ...
            bool inputWillBeProduced)
        {
            ...
            {
                parts.Add(T(
                    "PipelineReview.Guide.InputPendingDetail",
                    "An earlier enabled step will create this input when Review runs."));
            }
