...
namespace OpenVisionLab
{
    internal sealed class OpenVisionWorkspaceSampleLearnPathOption
    {
            ...
            IReadOnlyList<VisionPipelineSampleCatalogItem> samples)
        {
            ...
            AddIfNotEmpty(options, source, "threshold", LocalText("Threshold", "Threshold"), LocalText("이진화 기준과 Good/Bad 개수 판정", "Binary threshold and Good/Bad count gate"));
            AddIfNotEmpty(options, source, "filter", LocalText("Filter", "Filter"), LocalText("MedianBlur 노이즈 제거 후 Good/Bad 개수 판정", "MedianBlur denoise followed by a Good/Bad count gate"));
            AddIfNotEmpty(options, source, "morphology", LocalText("Morphology", "Morphology"), LocalText("Open 연산으로 작은 노이즈를 정리한 Good/Bad 개수 판정", "Opening cleanup followed by a Good/Bad count gate"));
            AddIfNotEmpty(options, source, "edge-detection", LocalText("Edge Detection", "Edge Detection"), LocalText("Canny edge map 뒤 Good/Bad 외곽선 개수 판정", "Canny edge map followed by a Good/Bad contour count gate"));
    ...
    internal static class OpenVisionWorkspaceSampleLearnPathClassifier
    {
        public static bool Matches(string learnPathId, VisionPipelineSampleCatalogItem sample)
        {
            ...
            {
                return ContainsAny(category, name, string.Empty, string.Empty, "Threshold");
            }

            if (string.Equals(id, "filter", StringComparison.OrdinalIgnoreCase))
            {
                return ContainsAny(category, name, string.Empty, string.Empty, "Filter");
            }

            if (string.Equals(id, "morphology", StringComparison.OrdinalIgnoreCase))
            {
                return ContainsAny(category, name, string.Empty, string.Empty, "Morphology");
            }

            if (string.Equals(id, "edge-detection", StringComparison.OrdinalIgnoreCase))
            {
                return ContainsAny(category, name, string.Empty, string.Empty, "EdgeDetection");
            }
            ...
            {
                return ContainsAny(category, name, string.Empty, string.Empty, "Blob");
            }

            if (string.Equals(id, "contour", StringComparison.OrdinalIgnoreCase))
            {
                return ContainsAny(category, name, string.Empty, string.Empty, "Contour");
            }

            if (string.Equals(id, "line", StringComparison.OrdinalIgnoreCase))
            {
                return ContainsAny(category, name, string.Empty, string.Empty, "Line");
            }
