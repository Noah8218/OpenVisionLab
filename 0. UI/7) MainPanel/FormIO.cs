...
{
    public partial class FormIO : WeifenLuo.WinFormsUI.Docking.DockContent
    {
        ...
        private void dgvDO_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                ...
                for (int nTotalCount = 0; nTotalCount < CDIO_CONTEC.Outputs.Count; nTotalCount++)
                {
                    if (!bContinue) { break; }

                    List<CSignal> Outputs = CDIO_CONTEC.Outputs;
                ...
                if (signal != null)
                {
                    string strName = dgvDO[DGV_NAME, e.RowIndex].Value.ToString();

                    if (strName == nameof(Global.Device.DIO_BD.DO_00_Encoder_OnOff))
                    {
                        Global.Device.DIO_PLC.WriteWord(Global.Device.DIO_PLC.DO_SOL_ON_REQUEST, 0);
                        Global.Device.DIO_PLC.WriteWord(Global.Device.DIO_PLC.DO_SOL_OFF_REQUEST, 0);
                        if (signal.Current == CSignal.SIGNAL_ON)
                        {
                            Global.Device.DIO_PLC.WriteWord(Global.Device.DIO_PLC.DO_SOL_OFF_REQUEST, 1);
                        }
                        else if (signal.Current == CSignal.SIGNAL_OFF)
                        {
                            Global.Device.DIO_PLC.WriteWord(Global.Device.DIO_PLC.DO_SOL_ON_REQUEST, 1);
                        }
                    }
        ...
        public bool InitUI()
        {
            List<CSignal> Inputs = CDIO_CONTEC.Inputs;
        ...
        public bool AddRowIO(CSignal signal, string strName)
        {
            try
            {
                ...
                if (strName == "Input")
                {
                    dgvDI.Rows.Add(signal.Address,signal.Name, signal.Current == CSignal.SIGNAL_ON ? "ON" : "OFF", signal.IS_CONTACT_A ? "A" : "B");
                }
                else if (strName == "Output")
                {
                    dgvDO.Rows.Add(signal.Address,signal.Name, signal.Current == CSignal.SIGNAL_OFF ? "ON" : "OFF");
                }
