...
{
    public partial class FormCircleProgress : Form
    {
        public EventHandler<EventArgs> EventExit;

        public FormCircleProgress()
        {
            InitializeComponent();
        }

        private void FormCircleProgress_Load(object sender, EventArgs e)
        {
            int nSize = 150;

            //15 로 전달 되어 있는 인자 -> 실제 모서리 둥글게 표현 하는 인자
            IntPtr ip = CUtil.CreateRoundRectRgn(0, 0, circularProgressBar5.Width, circularProgressBar5.Height, nSize, nSize);
            CUtil.SetWindowRgn(circularProgressBar5.Handle, ip, true);

            IntPtr ip2 = CUtil.CreateRoundRectRgn(0, 0, this.Width, this.Height, nSize, nSize);
