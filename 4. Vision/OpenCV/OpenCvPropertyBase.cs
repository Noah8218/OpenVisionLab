...
namespace OpenVisionLab.Vision._1._Tools.OpenCV
{
    ...
    public class OpenCvPropertyBase : DependencyObject, IOpenCVPropertyBase
    {
        ...
        protected T LoadConfigFile<T>(string RecipeName, Action<T> afterLoad) where T : OpenCvPropertyBase
        {
            T loaded = LoadConfigFile<T>(RecipeName);
        ...
        protected T LoadTestConfigFile<T>(string path, Action<T> afterLoad) where T : OpenCvPropertyBase
        {
            T loaded = LoadTestConfigFile<T>(path);
            afterLoad?.Invoke(loaded);
            return loaded;
        }
        ...
        {
            return RecipeWorkspaceService.GetVisionConfigPath(recipeName, NAME);
        }
        #endregion
    }
}
