...
namespace OpenVisionLab
{
    internal static class VisionPipelineArithmeticStep
    {
        ...
        {
            if (!IsArithmetic(step))
            {
                return false;
            }

            return RequiresInputLayerB(
                GetString(step?.Parameters, ParameterMode, ModeOperation),
                GetString(step?.Parameters, ParameterOperation, "Bitwise_AND"),
                UsesConstantInput(step));
        }

        public static bool RequiresInputLayerB(string mode, string operation, bool useConstantInput)
        {
            if (string.Equals(mode, ModeOffset, StringComparison.OrdinalIgnoreCase) || useConstantInput)
            {
                return false;
            }

            operation = string.IsNullOrWhiteSpace(operation) ? "Bitwise_AND" : operation;
