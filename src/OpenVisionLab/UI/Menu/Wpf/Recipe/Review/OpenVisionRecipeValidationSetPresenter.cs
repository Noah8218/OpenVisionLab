...
namespace OpenVisionLab
{
    ...
    {
        internal static OpenVisionRecipeValidationSetOptionSelection BuildOptionSelection(
        ...
        {
            IReadOnlyList<OpenVisionRecipeValidationSetOption> options = (document?.Sets ?? new List<OpenVisionRecipeValidationSet>())
                .Where(set => set != null)
                .OrderBy(set => set.Name, StringComparer.OrdinalIgnoreCase)
                .Select(set => new OpenVisionRecipeValidationSetOption(set))
                .ToList();
            OpenVisionRecipeValidationSetOption selected = FindOption(options, preferredName) ?? options.FirstOrDefault();
            return new OpenVisionRecipeValidationSetOptionSelection(
                options,
                selected,
                FindOption(options, preferredTrainName),
                FindOption(options, preferredValidationName),
                FindOption(options, preferredTestName));
        }

        internal static OpenVisionRecipeValidationSetImageSelection BuildImageSelection(
            OpenVisionRecipeValidationSetOption option,
            string previousPath)
        {
            IReadOnlyList<OpenVisionRecipeValidationSetImageRow> rows = (IReadOnlyList<OpenVisionRecipeValidationSetImageRow>)(option?.Set?.Images?
                .Where(image => image != null)
                .Select(image => new OpenVisionRecipeValidationSetImageRow(image))
                .ToList()
                ) ?? Array.Empty<OpenVisionRecipeValidationSetImageRow>();
            OpenVisionRecipeValidationSetImageRow selected = rows.FirstOrDefault(row =>
        ...
        private static OpenVisionRecipeValidationSetOption FindOption(
    ...
    internal sealed class OpenVisionRecipeValidationSetOptionSelection
    {
        internal OpenVisionRecipeValidationSetOptionSelection(
            IReadOnlyList<OpenVisionRecipeValidationSetOption> options,
            OpenVisionRecipeValidationSetOption selected,
        ...
        {
            Options = options ?? Array.Empty<OpenVisionRecipeValidationSetOption>();
            Selected = selected;
            Train = train;
        ...
        internal IReadOnlyList<OpenVisionRecipeValidationSetOption> Options { get; }
        internal OpenVisionRecipeValidationSetOption Selected { get; }
        internal OpenVisionRecipeValidationSetOption Train { get; }
        internal OpenVisionRecipeValidationSetOption Validation { get; }
        internal OpenVisionRecipeValidationSetOption Test { get; }
    }

    internal sealed class OpenVisionRecipeValidationSetImageSelection
    {
        internal OpenVisionRecipeValidationSetImageSelection(
            IReadOnlyList<OpenVisionRecipeValidationSetImageRow> rows,
            OpenVisionRecipeValidationSetImageRow selected)
        {
            Rows = rows ?? Array.Empty<OpenVisionRecipeValidationSetImageRow>();
            Selected = selected;
        }

        internal IReadOnlyList<OpenVisionRecipeValidationSetImageRow> Rows { get; }
        internal OpenVisionRecipeValidationSetImageRow Selected { get; }
    }
}
