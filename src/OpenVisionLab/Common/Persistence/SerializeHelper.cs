...
namespace OpenVisionLab
{
    ...
    internal enum XmlSaveFailureStage
    {
        BeforeWrite,
        BeforeReplace,
        Cleanup
    ...
    {
        private static readonly object saveFailureSync = new object();
        private static XmlSaveFailureStage? saveFailureStageForTest;

        internal static IDisposable BeginSaveFailureInjectionForTest(
            XmlSaveFailureStage stage)
        {
            lock (saveFailureSync)
            {
                XmlSaveFailureStage? previous = saveFailureStageForTest;
                saveFailureStageForTest = stage;
                return new SaveFailureInjectionScope(previous);
            }
        }
        ...
        internal static bool TryLoadFromXmlBytes<T>(
            byte[] bytes,
            out T value,
            out Exception loadException)
        {
            ...
            try
            {
                if (typeof(T) == typeof(VisionPipeline))
                {
                    VisionPipelineXmlSchemaPolicy.ThrowIfExecutionBlocked(bytes);
                }
        ...
        public static bool TryLoadFromXmlText<T>(string xmlText, out T value, out string errorMessage)
        {
            ...
            try
            {
                if (typeof(T) == typeof(VisionPipeline))
                {
                    VisionPipelineXmlSchemaPolicy.ThrowIfExecutionBlocked(xmlText);
                }
        ...
        public static bool SaveXmlFile<T>(string path, T value)
        {
            ...
            try
            {
                InjectSaveFailure(XmlSaveFailureStage.BeforeWrite);
                ...
                InjectSaveFailure(XmlSaveFailureStage.BeforeReplace);
            ...
            finally
            {
                ...
                {
                    InjectSaveFailure(XmlSaveFailureStage.Cleanup);
                    File.Delete(tempPath);
                }
            }
        }

        private static void InjectSaveFailure(XmlSaveFailureStage stage)
        {
            lock (saveFailureSync)
            {
                if (saveFailureStageForTest == stage)
                {
                    throw new IOException("Injected XML save failure at " + stage + ".");
                }
            }
        }
        ...
        private sealed class SaveFailureInjectionScope : IDisposable
        {
            private readonly XmlSaveFailureStage? previous;
            private bool disposed;

            internal SaveFailureInjectionScope(XmlSaveFailureStage? previous)
            {
            ...
            public void Dispose()
            {
                ...
                lock (saveFailureSync)
                {
                    saveFailureStageForTest = previous;
                    disposed = true;
                }
            }
        }
