...
namespace OpenVisionLab
{
    public static class AppPathService
    {
        ...
        public static string GetCaptureFilePath(string title, DateTime timestamp)
        {
            return CombineUnderRoot(
                CaptureDirectory,
                new[] { $"{title}_{timestamp:yyyyMMdd_HHmmss}.jpeg" },
                "Capture");
        }
        ...
        public static string GetTestConfigPath(string configName)
        {
            return CombineUnderRoot(
                TestDirectory,
                new[] { EnsureXmlExtension(configName) },
                "Test config");
        }
        ...
        private static string CombineUnderRoot(
            string root,
            string[] paths,
            string rootDescription)
        {
            if (ContainsTraversalSegment(paths))
            {
                throw new InvalidOperationException(
                    rootDescription
            ...
            string combined = Path.GetFullPath(Path.Combine(parts));
            if (!IsSameOrChildPath(root, combined))
            {
                throw new InvalidOperationException(
                    rootDescription
                    + " path escaped its intended root: "
                    + combined);
            }
        ...
        private static bool ContainsTraversalSegment(string[] paths)
        {
            if (paths == null)
            {
                return false;
