...
{
    public class TSMLightControler_TSM_P12R_V2
    {
        ...
        public TSMLightControler_TSM_P12R_V2()
        {
        ...
        //값 변환
        private void Convert(int ChannelCount, int nValues)
        {
            /// 채널 값
            string str = ChannelCount.ToString();
            ChannelCount = Int32.Parse(str);
        ...
        public bool On(int Channel)
        {
            try
            {
                if (SerialOpen)
                {
                    byte[] Arr_Channel = StringToByte(Channel.ToString());
                    byte[] Arr_On = new byte[7];

                    Arr_On[0] = m_Stx;
                    Arr_On[1] = m_Zero;
                    Arr_On[2] = Arr_Channel[0];
                    Arr_On[3] = m_O;
                    Arr_On[4] = m_N;
                    Arr_On[5] = m_On;
                    Arr_On[6] = m_Etx;
        ...
        public bool SetValue(int Channel, int Value)
        {
            try
            {
                ...
                byte[] control = new byte[9];

                control[0] = m_Stx;
                control[1] = m_Zero;
                control[2] = Arr_Channel[0];
                control[3] = m_W;
                control[4] = m_R;
                control[5] = Stval[0];
                control[6] = Stval[1];
        ...
        public bool ReadInitFile()
        {
            try
            {
                string strPath = Application.StartupPath + "\\CONFIG\\DEVICE\\" + m_XMLName + ".xml";
