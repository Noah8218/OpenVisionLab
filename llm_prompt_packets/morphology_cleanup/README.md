...
The desired response starts with `<?xml` and ends with `</VisionPipeline>`. The initial GPT request may include normal operator wording through the supplied prompt; the response itself must be XML-only so OpenVisionLab can validate it without manual cleanup.
...
- Target workflow: `Threshold -> Morphology(Open) -> Contour`.
- The Threshold output becomes the Morphology input; the Morphology output becomes the Contour input.
- Nominal image must pass exactly four cleaned target contours.
