...
namespace Lib.Line
{
    public static class VerticalLineCalculator
    {
        ...
        /// <param name=""></param>
        /// <returns></returns>
        public static List<LineSegment2D> GetVerticalLines(List<OpenCvSharp.Point> Edge, int ImageW, int ImageH, int POINT_RANGE, PROJECTION_DIR VER_PRJ_DIR)
        {
            // 수직선 라인들(직선의 방정식으로 만들어진)
            List<LineSegment2D> ver_lines = new List<LineSegment2D>();
            // 검출된 엣지의 숫자만큼 반복
            for (int Cnt = 0; Cnt < Edge.Count; Cnt++)
            {
                ...
                ver_lines.Add(new LineSegment2D(CommonConverter.PointToCVPoint(ptStart), CommonConverter.PointToCVPoint(ptEnd)));
            }
            return ver_lines;
        }
        ...
        /// <param name=""></param>
        /// <returns></returns>
        public static List<LineSegment2D> GetVerticalLinesManual(List<OpenCvSharp.Point> Edge, int ImageW, int ImageH, double MANUAL_ANGLE_VALUE, PROJECTION_DIR VER_PRJ_DIR)
        {
            // 수직선 라인들(직선의 방정식으로 만들어진)
            List<LineSegment2D> ver_lines = new List<LineSegment2D>();
            // 검출된 엣지의 숫자만큼 반복
            for (int Cnt = 0; Cnt < Edge.Count; Cnt++)
            {
                ...
                VerticalLineCalculator.GetLineCoef(Start, End, new OpenCvSharp.Rect(0, 0, ImageW, ImageH), VER_PRJ_DIR, result, out VerLine);
        ...
        public static List<LineSegment2D> GetIntersectionLines(List<LineSegment2D> ver_Lines, List<OpenCvSharp.Point> Edges)
        {
            List<LineSegment2D> intersectionLines = new List<LineSegment2D>();
            if (Edges.Count == 0)
            {
            ...
            List<PointF> Points_R = Edges.ConvertAll(new Converter<OpenCvSharp.Point, PointF>(CommonConverter.CVPointToPointF));

            for (int i = 0; i < ver_Lines.Count; i++)
            {
                LineSegment2D verLine = ver_Lines[i];
                ...
                if (intersectionPointArray.Length > 1) { intersectionLines.Add(new LineSegment2D(intersectionPointArray[0], intersectionPointArray[0 + 1])); }
            }

            return intersectionLines;
        }

        public static OpenCvSharp.Point GetIntersectionLines(LineSegment2D Lines_L, LineSegment2D Lines_R)
        {
