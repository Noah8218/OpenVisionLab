...
namespace OpenVisionLab._2._Common
{
    public class DefectList
    {
        ...
        public List<DefectList> GetProductsList()
        {
            var list = new List<DefectList>();           

            for(int i= 0; i < 100; i++)
            {
                DefectList cDefectList = new DefectList();
                cDefectList.No = i + 1;
        ...
        public BindingList<DefectList> GetDefectList(List<BlobResult> cResultBlobs, int cam_Index, int ImageH)
        {
            var list = new BindingList<DefectList>();
